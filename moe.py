...
class Routing(torch.nn.Module):
    ...
    def __init__(self, in_dim:int, n_experts:int, k:int, bs:int) -> None:
        ...
        self.W_noise = torch.nn.Linear(in_features=in_dim, out_features=n_experts, bias=0)
        self.k:int = k
    ...
    def forward(self, x:torch.tensor)-> torch.tensor:
        ...
        out = torch.nn.functional.softmax(self.W_g(x) 
                                          + torch.nn.functional.softplus(self.W_noise(x)))
...
class MoE(torch.nn.Module):
    ...
    def __init__(self, in_dim:int, out_dim:int,  n:int, k:int, bs:int) -> None:
        ...
        self.bs:int = bs
        self.experts = [Expert(in_dim=self.in_dim, out_dim=self.out_dim) for i in range(self.n)]
        self.routing = Routing(in_dim=self.in_dim, n_experts=self.n, k=self.k, bs=self.bs)
    ...
    def forward(self, x):
        ...
        for idx in range(self.bs):
            selected = filter(lambda i: self.experts.index(i) in indices[idx], self.experts)
